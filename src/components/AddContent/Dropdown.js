...
export default function Dropdown({selected,setSelected,categories}) {

  return (
    <Listbox value={selected} onChange={setSelected}>
      {({ open }) => (
        <>
          <Listbox.Label className="block  leading-6  text-xl font-bold mb-2">Category</Listbox.Label>
          <div className={`relative mt-2`}>
            <Listbox.Button  className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-1 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
              <span className="flex items-center">
                <span className="ml-3 block truncate" style={minw150}>{selected.name}</span>
              </span>
            </Listbox.Button>
